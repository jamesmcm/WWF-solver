...
class PopBoard(object):
    ...
    def __init__(self):
        try:
            self.sshot_img = Image.open("WWF1.jpg")
        except:
            print "Could not find image file"
        self.windowTopLeft = (0,0)
        self.windowToBoard = (0,0)
    ...
    def grabBoard(self, img):
        ...
        '''
        boardTL = (self.windowTopLeft[0] + self.windowToBoard[0], self.windowTopLeft[1] + self.windowToBoard[1])
        boardBR = (boardTL[0] + self.boardTLToBoardBR[0], boardTL[1] + self.boardTLToBoardBR[1])

        board = img.crop(boardTL+boardBR)
